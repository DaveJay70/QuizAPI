...
namespace QuizAPI.Controllers
{
    ...
    public class QuizzesController : ControllerBase
    {
        ...
        public IActionResult GetAllCount()
        {
            try
            {
                int quizzes = _repository.GetTotalQuizzesCount();
                return Ok(new { Quizzes_Count = quizzes });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { Error = "Failed to fetch quiz count", Details = ex.Message });
            }
        }
